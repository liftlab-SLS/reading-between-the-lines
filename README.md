...
The 10 witnesses used in the paper are: Catherine Jackson, Hugh O'Neill, Jane Williams, Jeffrey Kilper, John Adams, Kirk Dumont, Mark Pugh, Michael Wessler, Tiffany Kilper, and Todd Dean. Plain-text transcripts are included in `transcripts_txt/` (by UCSF document ID). Attorney question files extracted from those transcripts are in `attorney_questions/`.

Synthetic WitnessSim transcripts and delta logs for all 10 witnesses × 10 archetypes are included in `output/`.
...
| `transcripts_txt/` | Real UCSF deposition transcripts (by document ID) |
| `attorney_questions/` | Attorney question files per witness, used to drive WitnessSim |
| `output/` | Synthetic transcripts and delta logs (10 witnesses × 10 archetypes) |
