...
Download the transcripts as plain-text `.txt` files and place them in `data/`. The 10 witnesses used in the paper are: Catherine Jackson, Hugh O'Neill, Jane Williams, Jeffrey Kilper, John Adams, Kirk Dumont, Mark Pugh, Michael Wessler, Tiffany Kilper, and Todd Dean.

Synthetic transcripts are generated by WitnessSim (see Step 5 below). Attorney questions used to drive the simulator should be extracted from the same transcripts and provided as a zip of `.txt` files (one per witness deposition) passed to `batch_sim.py`.
...
| `data/` | Stories, emotion vectors, cached deposition scores |
| `output/` | Synthetic transcripts and delta logs from WitnessSim |
